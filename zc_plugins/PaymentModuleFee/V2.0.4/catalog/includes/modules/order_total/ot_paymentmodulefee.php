 ...
 * @version $Id: ot_paymentmodulefee.php zc158a - 2.2.2 PHP8.2  to 8.5 V2.0.4 BMH (OldNGreY) 2026-06-14
 * @maintained by oldngrey
 */
// 2026-06-14 fully encapsulated with installer to remove legacy files
...
if (!defined('VERSION_PMF')) {
    define('VERSION_PMF', '2.0.4');
}
...
class ot_paymentmodulefee
{
    ...
    function process()
    {
        ...
        if (MODULE_ORDER_TOTAL_PAYMENTMODULEFEE_FEE_ALLOW == 'true') {
            ...
            if (isset($_SESSION['payment'])) {          // ---- continue as payment type selected avoids PHP 8.0 error
                ...
                if (($pass == true) && in_array($_SESSION['payment'], $this->payment_modules)) {
                    ...
                    if ($charge_it == 'true') {
                        ...
                        if (substr($this->payment_fee, -1) == '%') {
                            ...
                            // 2024-04-01 $payment_module_fee = ($payment_subtotal_plus_shipping * ((int)$this->payment_fee/100));
                            ...
                            if (isset($ot_group_pricing->output)) {
                                if (isset($ot_group_pricing->output[0]['value'])) {
                                    ...
                                    // 2024-04-01 $payment_module_fee = ($payment_subtotal_plus_shipping_plus_coupon * ((int)$this->payment_fee/100));
                            ...
                            if ((isset($order->info['coupon_code'])) && (isset($order->info['coupon_amount']))) {
                                ...
                                $payment_subtotal_plus_shipping_plus_coupon = $order->info['subtotal'] + $order->info['shipping_cost'] - ($group_discountfee ?? 0) - ($order->info['coupon_amount'] ?? 0);
    ...
    function install()
    {
        ...
        values ('Sort Order', 'MODULE_ORDER_TOTAL_PAYMENTMODULEFEE_SORT_ORDER', '500', 'Sort order of display.', '6', '2', now())");
